...
class BackTestDriver(object):
	...
	# def testset_backtest(self,model,epoch,trade_parameter):
	# 	self.model = model
	# 	self.prediction_matrix = self.build_predictions(self.X_test_trans, self.Y_test_trans,self.Y_test_dates)
	# 	self.make_residual_hists(self.prediction_matrix,epoch,'Test')
	# 	self.backTestPanel = self.make_backtest_panel(self.prediction_matrix)

	# 	self.SignalThreshold = self.backTestPanel[ self.params['target_symbol'] ]['unmapped_score'].std()*trade_parameter# sigma threshold for signal to open position
	# 	self.AveThreshold = float(self.params['backtest_momentum_trade'])
	# 	self.close_parameter = self.SignalThreshold
	...
	# 	sim_params = create_simulation_parameters(start=self.sim_start,
	# 	                                          end=self.sim_end,
	...
	# 	# print(sim_params)
	# 	self.algo_obj = TradingAlgorithm(initialize=self.ZIPLINE_initialize,
	# 	                            handle_data=self.ZIPLINE_handle_correlations_aggressively,
	# 	                            sim_params=sim_params)

	# 	return self.algo_obj.run(self.backTestPanel)

	# def ZIPLINE_initialize(self,context):
	# 	df = fetch_csv(self.signalsPath, symbol_column="Symbol")
	# 	# set_benchmark( zipline_symbol("benchmark") )
	# 	context.set_commission(commission.PerShare(cost=0.0, min_trade_cost=0))
	    

	# # open at z=-K, close at z=0 or z=-2*K 
	# def ZIPLINE_handle_correlations_aggressively(self,context, data):
	#     # check for all symbols signal value
	    
	#     portfolio=context.portfolio
	#     record(positions=len(portfolio.positions.keys()))
	#     # print("Date = "+str(algo_obj.get_datetime()))
	#     for symb in [self.params['target_symbol'],]: 
	#         sec = zipline_symbol(symb)
	# #         debug(sec)
	#         if (data[sec].unmapped_score > self.SignalThreshold) and (sec not in viewkeys(portfolio.positions)):
	#         # if (data[sec].unmapped_score > self.SignalThreshold) and (float(data[sec]['target_MA20']) < -self.AveThreshold) and (sec not in viewkeys(portfolio.positions)):
	#             order_target_percent(sec, 0)
	#             order_target_percent(sec, 1.0) # long
	#             print("Lets buy "+symb+"  "+str(self.algo_obj.get_datetime()))
	#         if (data[sec].unmapped_score < -self.SignalThreshold) and (sec not in viewkeys(portfolio.positions)):
	#         # if (data[sec].unmapped_score < -self.SignalThreshold) and (float(data[sec]['target_MA20']) > self.AveThreshold) and (sec not in viewkeys(portfolio.positions)):
	#             order_target_percent(sec, 0)
	#             order_target_percent(sec, -1.0) # full portfolio loaded with symb
	#             print("Lets short "+symb+"  "+str(self.algo_obj.get_datetime()))
	...
	#         if  ((shares > 0) and ( data[asset].unmapped_score < self.close_parameter)) \
	#         or  ((shares < 0) and ( data[asset].unmapped_score > -1.0*self.close_parameter)) : # close position logic - cross 0 or StopLoss
	#             order_target_percent(asset, 0)
	#             print("Lets close "+str(asset)+"  "+str(self.algo_obj.get_datetime()))  
